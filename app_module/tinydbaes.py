...
class EncryptedJSONStorage(Storage):
    ...
    def read(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        if not size:
            ...
            crypt = pyaes.AESModeOfOperationCBC(self.encryption_key, iv=IV)
            decrypted_chunks = [b""] * (int(db_size / self.chunk_size) + 1)
            for i in range(0, db_size, self.chunk_size):
                decrypted_chunks[int(
                    i / self.chunk_size)] = crypt.decrypt(self._handle.read(self.chunk_size))
            ...
            for i in range(0, len(message), self.chunk_size):
                ...
                if len(chunk) % self.chunk_size != 0:
                    chunk += ' ' * (self.chunk_size - len(chunk) %
                                    self.chunk_size)
                self._handle.write(crypt.encrypt(chunk))
    ...
    def change_encryption_key(self, new_encryption_key):
        ...
        try:
            db_new_pw = TinyDB(encryption_key=new_encryption_key,
                               path=new_db_path, storage=EncryptedJSONStorage)
